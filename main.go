...
func funcionalitiestasksix(gtx C) {
	for {
		e, ok := gtx.Event(
			key.Filter{
				Focus: &task6Writer,
		...
		// Обработка нажатия Enter
		if ev.State == key.Press && ev.Name == key.NameReturn {
			Directory := task6Writer.Text() // Получаем текст из редактора

			destDir := "folderCopy" // Укажите путь к целевой директории

			err := syncDirectories(Directory, destDir)
			if err != nil {
				fmt.Println("Ошибка:", err)
			} else {
				fmt.Println("Синхронизированно!")
			}
		}
	}
}
...
func SeeMainWindow(th *material.Theme, gtx C) D {
	...
	return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
		...
		layout.Rigid(func(gtx C) D { //6 task
			StandardInset := layout.Inset{
				Top: unit.Dp(22),
			}
			return StandardInset.Layout(gtx, func(gtx C) D {
				lbl := material.Body1(th, "Синронизировать директории")
				lbl.TextSize = unit.Sp(14)
				return lbl.Layout(gtx)
			})
		}),
		layout.Rigid(func(gtx C) D {
			...
			return insetOut.Layout(gtx, func(gtx C) D {
				...
				return border.Layout(gtx, func(gtx C) D {
					...
					return inset.Layout(gtx, func(gtx C) D {
						userInput4 := material.Editor(th, &task6Writer, "Введите адрес директории(она будет скопирована в C:/Users/semen/OneDrive/Рабочий стол/VSC проекты/Много кнопок тренировка/folderCopy)")
						return userInput4.Layout(gtx)
					})
				})
			})
